...
sophiaClass returns[ClassDeclaration classDec] locals[FieldDeclaration field]:
            CLASS
            ...
            LBRACE
            (((vd1=varDeclaration {$field = new FieldDeclaration($vd1.varDec);
            $field.setLine($vd1.varDec.getVarName().getLine()); $classDec.addField($field);} 
            | m1=method {$classDec.addMethod($m1.methodDec);})*
            cc=constructor{$classDec.setConstructor($cc.constructorDec);}
            (vd2=varDeclaration {$field = new FieldDeclaration($vd2.varDec);
            $field.setLine($vd2.varDec.getVarName().getLine()); $classDec.addField($field);} 
            | m2=method {$classDec.addMethod($m2.methodDec);})*) |
            ((vd3=varDeclaration {$field = new FieldDeclaration($vd3.varDec);
            $field.setLine($vd3.varDec.getVarName().getLine()); $classDec.addField($field);} 
            | m2=method {$classDec.addMethod($m2.methodDec);})*))
            RBRACE;
...
listType returns [ListType list_type]: LIST LPAR
            ((INT_VALUE SHARP tp=type {$list_type = new ListType(Integer.parseInt($INT_VALUE.getText()), new ListNameType($tp.absType));})
...
methodCallStatement returns[MethodCallStmt methodcallstmnt]:
            mc=methodCall {$methodcallstmnt = new MethodCallStmt($mc.mCall); $methodcallstmnt.setLine($mc.mCall.getLine());} SEMICOLLON;
...
values returns[Value val]: bv=boolValue {$val = $bv.boolVal;}
            ...
            | INT_VALUE {$val = new IntValue(Integer.parseInt($INT_VALUE.getText())); $val.setLine($INT_VALUE.getLine());}
